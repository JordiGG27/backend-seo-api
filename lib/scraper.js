...
import { extractStructuredData } from './schemaValidator';
import { analyzeAIReadiness } from './geoAnalyzer';
import { estimateKeywordFocus } from './keywordFocusEstimator';
...
function extractLinks($, baseUrl) {
  ...
  return { internal, external, total: internal + external, internalUrls: [...internalUrls] };
}

export async function scrapeSEOData(targetUrl) {
  ...
  return {
    ...
    structuredData,
    keywordFocus: estimateKeywordFocus($, { title, headings }),
    geo: {
      aiReadiness: analyzeAIReadiness($, structuredData),
    },
    technical: {
      lang,
      canonical: {
